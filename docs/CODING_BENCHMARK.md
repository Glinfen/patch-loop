...
hard suite 和 advanced suite 使用 `hidden_tests` 为任务声明独立的源文件、工作区目标路径和 SHA-256。评测器在 Agent 停止后才校验并复制这些文件，Agent 的文件工具和测试过程无法提前读取；隐藏测试注入不会计入 Agent 变更范围。

`benchmarks/coding_tasks_advanced.json` 进一步覆盖版本化数据迁移、确定性灰度发布、跨文件原子转账，以及“修复实现并新增指定回归测试”的组合任务。逐轮结果与失败驱动优化记录见 `docs/CODING_BENCHMARK_ADVANCED_RESULTS.md`。
...
公开测试便于回放和诊断，但可能被模型针对。后续扩大套件时应继续增加运行时不可见测试、跨语言任务和真实仓库留出集。

当前 hard suite 和 advanced suite 已加入运行时不可见测试，但测试源码仍随项目发布，不属于保密留出集。
