...
| 4. [Workspace / Git 工作流](WORKSPACE_GIT_WORKFLOW_PLAN.md)     | FileChangeTracker 保存初始内容并输出 diff，尚无完整 Workspace Manager | dirty 基线、Session worktree、用户/Agent 变更归属、局部审查撤销、验证版本绑定提交 | 依赖 SRF 所有权与 Policy；必须先证明不会覆盖或误提交用户修改                     |
