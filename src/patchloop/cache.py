...
class CacheRequestFingerprint(BaseModel):
    ...
    def section_fingerprints(self) -> dict[str, str]:
        return {
            name: getattr(self, name).fingerprint
...
class CacheDiagnostics:
    ...
    def snapshot(self) -> CacheDiagnosticsSnapshot:
        ...
        return CacheDiagnosticsSnapshot(
            ...
            previous_section_lengths=(
                {
                    name: getattr(previous, name).byte_length
                    for name in _SECTION_ORDER
                }
                if previous
                else {}
    ...
    def _structural_reasons(
        ...
        if changed["system_instructions"] or (
            previous.dynamic_system_prefix.fingerprint
            != current.dynamic_system_prefix.fingerprint
        ):
...
def fingerprint_request(
    ...
    *,
    ...
    safe_messages = [
        redactor.redact(message.model_dump(mode="json")) for message in messages
    ]
    ...
    actual_project = (
        safe_messages[1].get("content", "") if len(safe_messages) > 1 else ""
    )
    ...
    sections = {
        name: _section_fingerprint(value)
...
def _primary_reason(reasons: Sequence[CacheLayoutReason]) -> CacheLayoutReason:
    return min(
        reasons,
        key=lambda reason: _REASON_PRIORITY.index(reason)
